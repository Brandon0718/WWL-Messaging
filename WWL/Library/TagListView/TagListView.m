...
@interface TagListView () <TagViewDelegate>
...
- (void)prepareForInterfaceBuilder {
    [self addTag:1 title:@"Thanks" imageUrl:nil];
    [self addTag:2 title:@"for" imageUrl:nil];
    [self addTag:3 title:@"using" imageUrl:nil];
    [self addTag:4 title:@"TagListView" imageUrl:nil];
}
...
- (TagView *)addTag:(long) identifier title:(NSString *)title imageUrl:(NSString*) imageUrl {
    TagView *tagView = [[TagView alloc] initWithTitle:identifier title:title imageUrl:imageUrl];
...
- (void)addTagsAccordingToDataSourceArray:(NSArray<NSString *> *)dataSourceArray withOnTapForEach:(void(^)(TagView *tagView))onTapBlock
{
    for (NSString *tagName in dataSourceArray) {
        TagView *tagView = [self addTag:0 title:tagName imageUrl:nil];
...
-(void)tagDeleteReceived:(long)identifier {
    [self removeTag:identifier];
    
    if(self.tagViewDelegate) {
        [self.tagViewDelegate tagDeleteReceived:identifier];
    }
}
...
- (void)removeTag:(long)identifier {
    ...
    for(NSInteger index = [[self tagViews] count] - 1 ; index >= 0; index--) {
        ...
        if(tagView.identifier == identifier) {
            [tagView removeFromSuperview];
